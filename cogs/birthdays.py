...
class Birthday_Modal(Modal, title="Défini ta date d'anniversaire"):
    ...
    async def on_submit(self, react: discord.Interaction) -> None:
        ...
        await execute_database(query="""INSERT INTO birthdays (discord_id, user_date) VALUES (?, ?)""", is_insert=True, data=[react.user.id, self.date.value])
...
class Birthday(commands.GroupCog, name="birthday"):
    ...
    async def check_birthdays(self):
        ...
        while not self.bot.is_closed():
            ...
            if now.hour == 15:
                list = await execute_database(query="""SELECT discord_id, user_date FROM birthdays""", is_insert=False)
...
async def execute_database(query: str, is_insert: bool, data: list = None) -> list:
    database = sqlite3.connect(paths.db_path)
    cur = database.cursor()
    if is_insert:
