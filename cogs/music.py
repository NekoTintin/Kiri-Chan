...
opts_playlist = {'extract_flat': 'in_playlist', 'logger': logger}

# Options lister les informations d'une vidéo
opts_video = {'format': 'bestaudio/best', 'postprocessors': [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'm4a'}], 'logger': logger}
	...
	async def _check_members_in_channel(self):
		...
		while is_active:
			if len(self.channel.members) <= 1:
				is_active = False
				channel = self.bot.get_channel(self.dict_of_player[self.guild.id].channel.id)
				msg = await channel.fetch_message(self.dict_of_player[self.guild.id].message_id)
				if (msg is not NONE):
					await msg.delete()
